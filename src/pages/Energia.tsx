...
// Preço escalonado pelo valor cobrado no TOI.
// Sem valor extraível, aplica-se o piso da vertical (49,90) — nunca o de 19,90.
const PRECO_BASE = 49.9;
const PRECO_ALTO = 79.9;
const LIMITE_VALOR = 1500;

const precoPara = (a: Analise | null): number =>
  a && typeof a.valor_cobrado === "number" && a.valor_cobrado > LIMITE_VALOR
    ? PRECO_ALTO
    : PRECO_BASE;
...
const fireCompraEnergia = (transactionId: string, preco: number) => {
  if (typeof window === "undefined") return;
  const chave = `energia_purchase_${transactionId}`;
  try {
  ...
  track("purchase", "energia_5_pagamento_confirmado", {
    transaction_id: transactionId,
    value: preco,
    currency: "BRL",
    items: [{ item_id: "contestacao_energia", item_name: "Contestacao TOI Energia Eletrica", price: preco, quantity: 1 }],
  });
};
// ─── TIPOS ─────────────────────────────────────────────────────────────────
interface Achado {
...
const LOADER_MESSAGES = [
  "Lendo o termo de ocorrência...",
  "Conferindo as formalidades da inspeção...",
  "Verificando o critério de cálculo aplicado...",
  "Avaliando o período retroativo cobrado...",
];

const TIPOS_AUTUACAO = [
  { id: "toi", name: "Termo de Ocorrência (TOI)", subtitle: "recebido na inspeção", icon: FileWarning },
  { id: "notificacao", name: "Notificação de cobrança", subtitle: "recuperação de consumo", icon: Receipt },
  { id: "fatura", name: "Fatura com valor retroativo", subtitle: "cobrança de meses anteriores", icon: Zap },
  { id: "outros", name: "Outros documentos", subtitle: "laudo, perícia ou correspondência", icon: PlusCircle },
];
...
useEffect(() => {
    ...
    const checkPaymentStatus = async () => {
      ...
      try {
        ...
        if (data.status === "approved") {
          ...
          setIsPaid(true);
          localStorage.setItem("energia_paid_status", "true");
          localStorage.removeItem("energia_pending_payment");
          fireCompraEnergia(paymentId.toString(), precoPara(analise));
          // Se voltou numa página "limpa" (recarga/aba nova), recupera a análise
          // do localStorage para que a defesa seja gerada.
          if (!analise) {
            const salvo = localStorage.getItem("energia_saved_result");
            if (salvo) {
              try {
  ...
  useEffect(() => {
    if (paymentId) return;
    try {
      const pendente = localStorage.getItem("energia_pending_payment");
      const jaPago = localStorage.getItem("energia_paid_status") === "true";
  ...
  const processFile = (file: File) => {
    track("energia_upload", "energia_2_documento_enviado", { file_type: file.type });
  ...
  const analisarAuto = async (base64Data: string, mimeType: string) => {
    ...
    try {
      const response = await fetch("/api/analyze-energia", {
        method: "POST",
      ...
      if (typeof data.result === "string") {
        ...
        if (lower.includes("documento_invalido")) {
          ...
          track("energia_analise_erro", "energia_erro_documento_invalido", { tipo: "documento_invalido" });
          throw new Error("O arquivo enviado não parece ser um TOI, uma notificação de recuperação de consumo ou uma fatura com cobrança retroativa. Confira o documento e tente novamente.");
        }
      ...
      if (!resultado.houve_achado || !resultado.achados || resultado.achados.length === 0) {
        isBusinessError = true;
        track("energia_analise_inviavel", "energia_3_sem_falha", { motivo: "sem_falha" });
      ...
      track("energia_analise_viavel", "energia_3_paywall_exibido", { viabilidade: v ? v.nivel : "Negada" });
      setHasAnalyzed(true);
      localStorage.setItem("energia_saved_result", JSON.stringify(resultado));
      setIsResultModalOpen(true);
    } catch (err: any) {
      console.error("Erro na Análise de Energia:", err);
    ...
    try {
      const response = await fetch("/api/create-payment", {
        ...
        body: JSON.stringify({
          ...
          valor: preco,
          descricao: "Contestacao TOI Energia Eletrica - CheckMulta",
        }),
      });
      ...
      if (response.ok && data.qr_code) {
        track("begin_checkout", "energia_4_checkout_iniciado", { value: preco, currency: "BRL" });
        setPaymentId(data.id);
        try {
          localStorage.setItem("energia_pending_payment", JSON.stringify({
            id: data.id,
            ts: Date.now(),
          }));
        } catch {}
  ...
  const generateDefense = async (overrideAnalise?: Analise) => {
    ...
    try {
      const response = await fetch("/api/generate-defense-energia", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setShowFomoBanner(false);
      /* NAO limpar o storage aqui: se o usuario fechar a aba agora, ele perde
         a peca que acabou de pagar. Guardamos o texto final e mantemos
         saved_result e paid_status para permitir a retomada. */
      try {
        localStorage.setItem("energia_defense_result", data.result);
      } catch {
        /* storage cheio ou indisponivel: o usuario ainda ve o texto na tela */
      }
      localStorage.removeItem("energia_pending_payment");
    } catch (err: any) {
      clearTimeout(timeoutId);
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      track("energia_defesa_copiada", "energia_6_defesa_copiada");
      setIsCopied(true);
  ...
  const handleDownload = () => {
    if (!defenseResult) return;
    track("energia_defesa_baixada", "energia_6_defesa_baixada");
    const blob = new Blob([defenseResult], { type: "text/plain" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "contestacao-toi-energia.txt";
    a.click();
        ...
        <div className="mx-auto max-w-5xl px-4 py-16">
          <div className="mb-10 text-center">
            ...
            <p className="mx-auto max-w-2xl text-base leading-relaxed text-slate-600">
              ...
              duas frentes: as formalidades da inspeção e o cálculo do valor cobrado.
            </p>
          </div>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {[
              { t: "Entrega do TOI com recibo", d: "Cópia legível entregue ao consumidor ou a quem acompanhou a inspeção, mediante recibo assinado, na forma do art. 591, I." },
              { t: "Acompanhamento da inspeção", d: "Termo lavrado apenas pelos inspetores, sem consumidor nem acompanhante, compromete o contraditório previsto no art. 590." },
              { t: "Direito à perícia no medidor", d: "O TOI deve informar a possibilidade de perícia metrológica no INMETRO, os prazos e os custos envolvidos (art. 591, II)." },
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            <Reveal delay={0} className="rounded-xl border border-slate-200 bg-slate-50/60 p-6">
              ...
              <p className="text-[15.5px] leading-relaxed text-slate-600">
                O prazo consta da própria notificação que você recebeu. Confira o
                documento e, em caso de dúvida, confirme junto à distribuidora.
            ...
            <Reveal delay={0.06} className="rounded-xl border border-slate-200 bg-slate-50/60 p-6">
              ...
              <p className="text-[15.5px] leading-relaxed text-slate-600">
                O débito é lançado na fatura e passa a ser cobrado como dívida
            ...
            <Reveal delay={0} className="rounded-xl border border-slate-200 bg-slate-50/60 p-6">
              ...
              <p className="text-[15.5px] leading-relaxed text-slate-600">
                TOI lavrado sem o consumidor nem acompanhante, alegação de violação
                do medidor sem avaliação técnica, perícia em laboratório sem aviso
            ...
            <Reveal delay={0} className="rounded-xl border border-slate-200 bg-slate-50/60 p-6 md:col-span-2">
              ...
              <p className="text-[15.5px] leading-relaxed text-slate-600">
                Não na via administrativa: o próprio consumidor protocola a
                reclamação na distribuidora e, se necessário, na ouvidoria e na
        ...
        <div className="mx-auto max-w-3xl px-4 py-16">
          ...
          <Reveal className="max-w-none" amount={0.15}>
            <p className="mb-4 text-[16.5px] leading-[1.75] text-slate-700">
              ...
              e cobra retroativamente a energia que entende não ter sido
              registrada. Essa cobrança não é automática: ela só se sustenta se o
              procedimento tiver observado as exigências da regulação federal.
            </p>
            ...
            <p className="mb-4 text-[16.5px] leading-[1.75] text-slate-700">
              O art. 590 da Resolução ANEEL nº 1.000/2021 exige um conjunto de
              providências para caracterizar o procedimento irregular: emitir o TOI
              em formulário próprio, solicitar a verificação ou perícia metrológica,
              elaborar relatório de avaliação técnica quando houver violação do
              medidor e avaliar o histórico de consumo. O art. 591 acrescenta a
              entrega de cópia legível mediante recibo assinado e a informação
              expressa sobre o direito de perícia no INMETRO, com prazos e custos.
            </p>

            <h3 className="mb-3 mt-9 text-xl font-bold leading-snug text-slate-900 sm:text-[22px]">
              O erro mais comum: o período cobrado
            </h3>
            <p className="mb-4 text-[16.5px] leading-[1.75] text-slate-700">
            ...
            <p className="mb-4 text-[16.5px] leading-[1.75] text-slate-700">
              Por se tratar de relação de consumo, cabe à distribuidora comprovar
              que adotou as providências necessárias para apurar o consumo não
              faturado e que assegurou o contraditório ao consumidor. O
              descumprimento das formalidades da Resolução ANEEL nº 1.000/2021 na
              emissão do TOI tem levado, na jurisprudência, à declaração de
              nulidade do débito dele decorrente.
            </p>

            <h3 className="mb-3 mt-9 text-xl font-bold leading-snug text-slate-900 sm:text-[22px]">
              Onde protocolar a contestação
            </h3>
            <p className="mb-4 text-[16.5px] leading-[1.75] text-slate-700">
              A reclamação começa na própria distribuidora, pelos canais de
              atendimento, e o número de protocolo deve ser guardado. Se a resposta
              for negativa ou não vier, o caminho seguinte é a ouvidoria da
              distribuidora e, depois, a ANEEL. A via judicial permanece disponível
              em qualquer momento.
            </p>
            ...
            <p className="mb-4 text-[16.5px] leading-[1.75] text-slate-700">
              O CheckMulta analisa gratuitamente o seu documento e aponta as falhas
              encontradas, sempre citando o trecho exato que fundamenta cada
              apontamento. Havendo falha, geramos uma{" "}
              <strong className="font-semibold text-slate-900">
      ...
      <footer className="border-t border-slate-200 bg-white">
        <div className="mx-auto max-w-4xl px-4 py-10 text-center">
          ...
          <p className="mx-auto max-w-3xl text-xs leading-relaxed text-slate-500">
            ...
            nosso sistema atua como organizador tecnológico com base na Resolução
            ANEEL nº 1.000/2021 e no Código de Defesa do Consumidor. Não prestamos
            consultoria jurídica nem representação. A decisão sobre a reclamação é
            da distribuidora e, em grau seguinte, da ANEEL. Não exigimos cadastro e
            não armazenamos o seu documento.
          </p>
            ...
            <div className="min-h-full flex items-center justify-center p-4 sm:p-6">
              ...
              >
                ...
                <div className="space-y-3 text-[15px] leading-relaxed text-slate-600">
                  ...
                  {activeModal === "privacidade" && <p>Sua privacidade é absoluta. Não possuímos banco de dados, nem realizamos registros do documento enviado, dos seus dados ou da contestação gerada. O processamento é de caráter transitório para elaboração do documento, que é imediatamente apagado após o fechamento da página.</p>}
                  {activeModal === "suporte" && (
                ...
                <div className="mt-4 w-full space-y-6">
                    ...
                    <div className="mx-auto flex max-w-md flex-col items-center space-y-5 py-4 text-center">
                      ...
                      <div>
                        ...
                        <p className="leading-relaxed text-slate-600">
                          Analisamos o documento ponto por ponto e <strong className="font-semibold text-slate-900">não encontramos falha</strong> entre os pontos verificados. O procedimento aparenta seguir as exigências da Resolução ANEEL nº 1.000/2021.
                        </p>
                  ...
                  {analise && analise.houve_achado && !isPaid && !isAnalyzing && !error && !rejeicaoInfo && (
                    <div className="space-y-6">
                      ...
                      {/* Dados do processo */}
                      {(analise.titular || analise.distribuidora || analise.unidade_consumidora) && (
                        <div className="rounded-lg border border-slate-200 bg-slate-50 p-5">
                          ...
                          {analise.cobranca_identificada && (
                            <div className="mt-4 flex items-start gap-3 border-t border-slate-200 pt-4">
                              ...
                              <div>
                                <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Cobrança apurada</p>
                                <p className="mt-0.5 text-[13px] leading-relaxed text-slate-700">{analise.cobranca_identificada}</p>
                              </div>
                            </div>
                          )}
                        </div>
                      ...
                      {viabilidade?.nivel === "Baixa" && (
                        <div className="rounded-lg border border-amber-200 bg-amber-50/60 p-5 text-left">
                          ...
                          <p className="text-sm leading-relaxed text-slate-700">
                            ...
                            contestação, mas não é do tipo que costuma levar à anulação direta.
                            Ainda assim, contestar tem valor: a distribuidora passa a ter de
                            responder de forma fundamentada, e a reclamação em aberto é o que
                            sustenta o pedido de suspensão da cobrança.
                          </p>
                    ...
                    <div className="flex flex-col space-y-6">
                      ...
                      <div className="flex flex-col items-center gap-4 border-t border-slate-200 pt-6">
                        <div className="flex w-full flex-col items-center justify-center gap-4 sm:flex-row">
                          ...
                          <GerarPdfDefesa
                            texto={defenseResult}
                            nomeArquivo="contestacao-toi-energia"
                            corBotao="bg-amber-600 hover:bg-amber-700"
                            onBaixar={() => track("energia_defesa_baixada_pdf", "energia_6_defesa_baixada_pdf")}
                          />
                        </div>
                        <button onClick={() => setActiveModal("suporte")} className="text-sm text-slate-400 transition hover:text-amber-600">
                          Precisa de ajuda? Fale com o suporte.
          ...
          <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
            ...
            >
              ...
              <p className="mb-6 text-sm leading-relaxed text-slate-600">
                Encontramos a análise do documento
